...
export default () => {
  ...
  const latestArticle = articles.length ? articles[articles.length - 1];

  return (
    <div className={commonStyles.content}>
      <div className={styles.mainNews}>
        <div>
          <img src={`${Config.contentUrl}${articles[articles.length][0].formats.thumbnail.url}`}/>
        </div>
        <span>{articles.length ? articles[articles.length - 1]["Title"] : ""}</span>
      </div>
